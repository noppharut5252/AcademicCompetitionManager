...
import { Save, X, Image as ImageIcon, Plus, Trash2, LayoutTemplate, PenTool, Upload, Loader2, Hash, Type, BoxSelect, ArrowUpFromLine, ArrowDownToLine, Scaling, MoveVertical, QrCode, Eye, ChevronDown, ChevronUp } from 'lucide-react';
...
const DEFAULT_TEMPLATE: CertificateTemplate = {
    ...
    showRank: false, // Judges don't have ranks
    serialFormat: 'JUDGE-{year}-{run:4}',
    serialStart: 1,
    contentTop: 25,
    footerBottom: 25,
    logoHeight: 35,
...
const JudgeCertificateConfigModal: React.FC<CertificateConfigModalProps> = ({ isOpen, onClose, data, onSave, initialTemplates, currentUser }) => {
  ...
  // Use 'judge_' prefix for context IDs to separate from student certs
  const getContextId = (rawId: string) => `judge_${rawId}`;
  const getRawId = (contextId: string) => contextId.replace('judge_', '');

  const [selectedContext, setSelectedContext] = useState<string>(isAdminOrArea ? 'judge_area' : (userClusterID ? `judge_${userClusterID}` : 'judge_area'));
  ...
  useEffect(() => {
      ...
      if (existing) {
          setCurrentTemplate({ ...DEFAULT_TEMPLATE, ...existing });
      } else {
          const rawId = getRawId(selectedContext);
          const clusterName = data.clusters.find(c => c.ClusterID === rawId)?.ClusterName || 'ระดับเขตพื้นที่';
          setCurrentTemplate({
              ...DEFAULT_TEMPLATE,
              id: selectedContext,
              name: rawId === 'area' ? 'เกียรติบัตรกรรมการ (ระดับเขต)' : `เกียรติบัตรกรรมการ (${clusterName})`
          });
      }
      ...
      try {
          ...
          if (targetFieldForUpload === 'backgroundUrl') {
              if (file.size > 2 * 1024 * 1024) { 
                  base64 = await resizeImage(file, 2480, 1748, 0.9, 'image/jpeg'); 
              } else {
                  base64 = await fileToBase64(file);
              }
              filename = `judge_cert_bg_${selectedContext}.jpg`;
          } else {
              base64 = await resizeImage(file, 800, 800, 0.9, 'image/png');
              filename = `judge_cert_logo_${selectedContext}_${targetFieldForUpload}.png`;
          }
  ...
  const handlePreview = async () => {
      ...
      let qrCodeBase64 = '';
      try {
          qrCodeBase64 = await QRCode.toDataURL("https://example.com/verify-judge", { margin: 1, width: 300 });
      } catch (e) {}
      ...
      const htmlContent = `
        ...
        <style>
            ...
            .header { font-size: 24pt; font-weight: bold; color: #1e3a8a; margin-bottom: 5mm; text-align: center; line-height: 1.2; font-family: '${template.fontHeader || defaultFont}', sans-serif; }
            .subheader { font-size: 16pt; margin-bottom: 8mm; text-align: center; font-family: '${template.fontSubHeader || defaultFont}', sans-serif; }
            .name { font-size: 32pt; font-weight: bold; color: #111; margin-bottom: 5mm; font-family: '${template.fontName || defaultFont}', sans-serif; text-align: center; border-bottom: 2px dotted #ccc; padding: 0 20px; min-width: 50%; }
            .desc { font-size: 16pt; margin-bottom: 5mm; max-width: 80%; text-align: center; line-height: 1.5; font-family: '${template.fontDesc || defaultFont}', sans-serif; }
            .highlight { font-weight: bold; color: #2563eb; }
            .date { font-size: 14pt; margin-top: auto; margin-bottom: 10mm; font-family: '${template.fontDate || defaultFont}', sans-serif; }
            ...
            .sig-name { font-size: 12pt; font-weight: bold; padding-top: 2px; width: 100%; margin-top: ${template.signatureSpacing || 3}mm; font-family: '${template.fontSignatures || defaultFont}', sans-serif; }
            .sig-pos { font-size: 10pt; white-space: pre-line; line-height: 1.3; margin-top: 2px; font-family: '${template.fontSignatures || defaultFont}', sans-serif; }
          ...
          <div class="page">
              ...
              <div class="content">
                  ...
                  <div class="name ${shadowClass}">นายกรรมการ ตัดสินเที่ยงตรง</div>
                  <div class="desc ${shadowClass}">
                      ได้ปฏิบัติหน้าที่เป็น <span class="highlight">กรรมการตัดสินการแข่งขัน</span><br/>
        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col relative">
            <div className="bg-indigo-600 px-6 py-4 flex justify-between items-center text-white shrink-0">
                <h3 className="text-lg font-bold flex items-center">
                    ...
                    ตั้งค่าเกียรติบัตรกรรมการ (Judge Certificates)
                </h3>
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50">
                <div className="mb-6 bg-white p-4 rounded-xl border border-indigo-100 shadow-sm">
                    ...
                    >
                        {isAdminOrArea && <option value="judge_area">ระดับเขตพื้นที่การศึกษา (District)</option>}
                        <optgroup label="ระดับกลุ่มเครือข่าย (Cluster Level)">
                            {data.clusters.map(c => {
                                if (isGroupAdmin && c.ClusterID !== userClusterID) return null;
                                return <option key={c.ClusterID} value={`judge_${c.ClusterID}`}>{c.ClusterName}</option>
                            })}
                        </optgroup>
                    </select>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        <h4 className="font-bold text-gray-800 border-b pb-2 mb-2 flex items-center"><ImageIcon className="w-4 h-4 mr-2" /> ตั้งค่าพื้นหลังและโลโก้</h4>
                        
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1">รูปพื้นหลัง (Background)</label>
                            <div className="flex gap-2">
                                <input className="flex-1 border rounded px-3 py-2 text-sm" placeholder="URL..." value={currentTemplate.backgroundUrl} onChange={(e) => updateField('backgroundUrl', e.target.value)} />
                                <button onClick={() => triggerUpload('backgroundUrl')} disabled={uploadingState.loading} className="px-3 py-2 bg-gray-100 rounded border hover:bg-gray-200">
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
                            <label className="block text-xs font-medium text-gray-600 mb-2 flex items-center"><BoxSelect className="w-3.5 h-3.5 mr-1" /> รูปแบบกรอบ (Frame Style)</label>
                            <div className="grid grid-cols-2 gap-2">
                                <button className={`p-2 rounded border text-xs text-center transition-colors ${currentTemplate.frameStyle === 'simple-gold' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold' : 'bg-white border-gray-200 hover:bg-gray-100'}`} onClick={() => updateField('frameStyle', 'simple-gold')}>เส้นเดี่ยว (Gold)</button>
                                <button className={`p-2 rounded border text-xs text-center transition-colors ${currentTemplate.frameStyle === 'infinite-wave' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold' : 'bg-white border-gray-200 hover:bg-gray-100'}`} onClick={() => updateField('frameStyle', 'infinite-wave')}>เส้นคลื่น (Wave)</button>
                                <button className={`p-2 rounded border text-xs text-center transition-colors ${currentTemplate.frameStyle === 'ornamental-corners' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold' : 'bg-white border-gray-200 hover:bg-gray-100'}`} onClick={() => updateField('frameStyle', 'ornamental-corners')}>ขอบลายไทย</button>
                                {getRawId(selectedContext) === 'area' && <button className={`p-2 rounded border text-xs text-center transition-colors ${currentTemplate.frameStyle === 'thai-premium' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold' : 'bg-white border-gray-200 hover:bg-gray-100'}`} onClick={() => updateField('frameStyle', 'thai-premium')}>ลายไทยพรีเมียม</button>}
                                <button className={`p-2 rounded border text-xs text-center transition-colors ${currentTemplate.frameStyle === 'none' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 font-bold' : 'bg-white border-gray-200 hover:bg-gray-100'}`} onClick={() => updateField('frameStyle', 'none')}>ไม่มีกรอบ</button>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-xs font-medium text-gray-600 mb-1">โลโก้ซ้าย</label>
                                <div className="flex gap-1">
                                    <input className="flex-1 border rounded px-2 py-2 text-sm min-w-0" placeholder="URL..." value={currentTemplate.logoLeftUrl} onChange={(e) => updateField('logoLeftUrl', e.target.value)} />
                                    <button onClick={() => triggerUpload('logoLeftUrl')} disabled={uploadingState.loading} className="px-2 py-2 bg-gray-100 rounded border hover:bg-gray-200">{uploadingState.loading && uploadingState.field === 'logoLeftUrl' ? <Loader2 className="w-4 h-4 animate-spin"/> : <Upload className="w-4 h-4"/>}</button>
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-600 mb-1">โลโก้ขวา</label>
                                <div className="flex gap-1">
                                    <input className="flex-1 border rounded px-2 py-2 text-sm min-w-0" placeholder="URL..." value={currentTemplate.logoRightUrl} onChange={(e) => updateField('logoRightUrl', e.target.value)} />
                                    <button onClick={() => triggerUpload('logoRightUrl')} disabled={uploadingState.loading} className="px-2 py-2 bg-gray-100 rounded border hover:bg-gray-200">{uploadingState.loading && uploadingState.field === 'logoRightUrl' ? <Loader2 className="w-4 h-4 animate-spin"/> : <Upload className="w-4 h-4"/>}</button>
                                </div>
                            </div>
                        </div>

                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1">หัวเรื่อง (Header)</label>
                            <input className="w-full border rounded px-3 py-2 text-sm" value={currentTemplate.headerText} onChange={(e) => updateField('headerText', e.target.value)} />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1">ข้อความรอง (Sub Header)</label>
                            <input className="w-full border rounded px-3 py-2 text-sm" value={currentTemplate.subHeaderText} onChange={(e) => updateField('subHeaderText', e.target.value)} />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1">ชื่องาน (Event Name)</label>
                            <input className="w-full border rounded px-3 py-2 text-sm" placeholder="เว้นว่างเพื่อใช้ค่าเริ่มต้น" value={currentTemplate.eventName || ''} onChange={(e) => updateField('eventName', e.target.value)} />
                        </div>
                        <div>
                            <label className="block text-xs font-medium text-gray-600 mb-1">ข้อความวันที่</label>
                            <input className="w-full border rounded px-3 py-2 text-sm" value={currentTemplate.dateText} onChange={(e) => updateField('dateText', e.target.value)} />
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white p-3 rounded-lg border border-purple-200 shadow-sm">
                            <h4 className="font-bold text-gray-800 border-b pb-2 mb-2 flex items-center text-sm"><Type className="w-4 h-4 mr-2 text-purple-500" /> รูปแบบตัวอักษร</h4>
                            <div className="space-y-3">
                                <div>
                                    <label className="block text-xs font-medium text-gray-600 mb-1">ฟอนต์หลัก</label>
                                    <select className="w-full border rounded px-2 py-1.5 text-sm outline-none" value={currentTemplate.fontFamily || 'Sarabun'} onChange={(e) => updateField('fontFamily', e.target.value)}>
                                        {FONT_OPTIONS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
                                    </select>
                                </div>
                                <div className="flex items-center mb-2">
                                    <input type="checkbox" id="enableShadow" className="w-4 h-4 text-purple-600 rounded border-gray-300" checked={currentTemplate.enableTextShadow !== false} onChange={(e) => updateField('enableTextShadow', e.target.checked)} />
                                    <label htmlFor="enableShadow" className="ml-2 text-xs text-gray-700">เปิดใช้งานเงาตัวอักษร</label>
                                </div>
                                <button onClick={() => setShowAdvancedFonts(!showAdvancedFonts)} className="text-xs text-purple-600 font-bold hover:underline flex items-center mb-1">
                                    {showAdvancedFonts ? <ChevronUp className="w-3 h-3 mr-1"/> : <ChevronDown className="w-3 h-3 mr-1"/>} {showAdvancedFonts ? 'ซ่อนตั้งค่าขั้นสูง' : 'ตั้งค่าฟอนต์แยกส่วน'}
                                </button>
                                {showAdvancedFonts && (
                        ...
                        <div className="bg-white p-3 rounded-lg border border-blue-200 shadow-sm">
                            <h4 className="font-bold text-gray-800 border-b pb-2 mb-2 flex items-center text-sm"><Scaling className="w-4 h-4 mr-2 text-blue-500" /> การจัดวาง (mm)</h4>
                            <div className="grid grid-cols-2 gap-3 mb-3">
                                <div><label className="block text-[10px] font-medium text-gray-600 mb-1 flex items-center"><ArrowDownToLine className="w-3 h-3 mr-1"/> ขอบบน</label><input type="number" className="w-full border rounded px-2 py-1.5 text-sm" value={currentTemplate.contentTop || 25} onChange={(e) => updateField('contentTop', parseInt(e.target.value))} /></div>
                                <div><label className="block text-[10px] font-medium text-gray-600 mb-1 flex items-center"><ArrowUpFromLine className="w-3 h-3 mr-1"/> ขอบล่าง</label><input type="number" className="w-full border rounded px-2 py-1.5 text-sm" value={currentTemplate.footerBottom || 25} onChange={(e) => updateField('footerBottom', parseInt(e.target.value))} /></div>
                                <div><label className="block text-[10px] font-medium text-gray-600 mb-1 flex items-center"><Scaling className="w-3 h-3 mr-1"/> สูงโลโก้</label><input type="number" className="w-full border rounded px-2 py-1.5 text-sm" value={currentTemplate.logoHeight || 35} onChange={(e) => updateField('logoHeight', parseInt(e.target.value))} /></div>
                                <div><label className="block text-[10px] font-medium text-gray-600 mb-1 flex items-center"><MoveVertical className="w-3 h-3 mr-1"/> ห่างลายเซ็น</label><input type="number" className="w-full border rounded px-2 py-1.5 text-sm" value={currentTemplate.signatureSpacing || 3} onChange={(e) => updateField('signatureSpacing', parseInt(e.target.value))} /></div>
                            </div>
                        </div>
                        
                        {/* Serial Number Configuration */}
                        <div className="bg-white p-3 rounded-lg border border-orange-100 shadow-sm">
                            <h4 className="font-bold text-gray-800 border-b pb-2 mb-2 flex items-center text-sm"><Hash className="w-4 h-4 mr-2 text-orange-500" /> เลขทะเบียน (Serial)</h4>
                            <div className="space-y-3">
                                <div>
                        ...
                        <div>
                            <div className="flex justify-between items-center border-b pb-2 mb-2">
                                <h4 className="font-bold text-gray-800 flex items-center"><PenTool className="w-4 h-4 mr-2" /> ผู้ลงนาม</h4>
                                <div className="flex items-center"><input type="checkbox" className="w-4 h-4" checked={currentTemplate.showSignatureLine !== false} onChange={(e) => updateField('showSignatureLine', e.target.checked)} /><label className="ml-2 text-xs text-gray-600">เส้นบรรทัด</label></div>
                            </div>
                            <div className="space-y-4 max-h-[150px] overflow-y-auto pr-2 custom-scrollbar">
                                {currentTemplate.signatories.map((sig, idx) => (
                                    <div key={idx} className="bg-white p-3 rounded-lg border border-gray-200 relative">
                                        <button onClick={() => removeSignatory(idx)} className="absolute top-2 right-2 text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                                        <div className="space-y-2">
                                            <input type="text" className="w-full border rounded px-2 py-1 text-sm" placeholder="ชื่อ-นามสกุล" value={sig.name} onChange={(e) => updateSignatory(idx, 'name', e.target.value)} />
                                            <textarea className="w-full border rounded px-2 py-1 text-sm resize-none" placeholder="ตำแหน่ง" rows={2} value={sig.position} onChange={(e) => updateSignatory(idx, 'position', e.target.value)} />
                                            <div className="flex gap-2">
                                                <input type="text" className="flex-1 border rounded px-2 py-1 text-sm" placeholder="URL ลายเซ็นต์" value={sig.signatureUrl} onChange={(e) => updateSignatory(idx, 'signatureUrl', e.target.value)} />
                                                <button onClick={() => triggerUpload(`signatory-${idx}`)} disabled={uploadingState.loading} className="px-2 py-1 bg-gray-100 rounded border hover:bg-gray-200">{uploadingState.loading && uploadingState.field === `signatory-${idx}` ? <Loader2 className="w-4 h-4 animate-spin"/> : <Upload className="w-4 h-4"/>}</button>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            <button onClick={addSignatory} className="w-full mt-3 py-2 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-blue-500 hover:text-blue-500 transition-colors flex items-center justify-center text-sm font-medium"><Plus className="w-4 h-4 mr-1" /> เพิ่มผู้ลงนาม</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-gray-100 px-6 py-4 flex justify-between gap-3 shrink-0">
                <button onClick={handlePreview} className="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 flex items-center shadow-sm"><Eye className="w-4 h-4 mr-2" /> Preview</button>
                <div className="flex gap-2">
                    <button onClick={onClose} className="px-4 py-2 text-gray-600 hover:bg-gray-200 rounded-lg text-sm font-medium">ปิด</button>
                    <button onClick={handleSaveAll} disabled={isSaving} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium flex items-center shadow-sm disabled:opacity-70">{isSaving ? 'กำลังบันทึก...' : <><Save className="w-4 h-4 mr-2" /> บันทึกการตั้งค่า</>}</button>
                </div>
            </div>
        </div>
    </div>
...
export default JudgeCertificateConfigModal;
