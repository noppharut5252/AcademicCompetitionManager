...
const JudgesView: React.FC<JudgesViewProps> = ({ data, user, onDataUpdate }) => {
  ...
  const activityGroups = useMemo(() => {
      ...
      // 3. Map Judges with Visibility Filter
  ...
  const handleImportFromCluster = (clusterJudge: Judge) => {
      ...
      setEditingJudge({
          ...
          id: undefined // CRITICAL: Treat as NEW RECORD, do NOT carry over ID
      });
      // Force Area Scope for imported judge
  ...
  const handleFinalSave = async () => {
      ...
      try {
          ...
          const promises = assignments.map(assign => {
              ...
              const payload: Judge = {
                  ...baseJudge as Judge,
                  // New ID logic: Must include Scope to avoid collision between Area/Cluster judges with same name/activity
                  id: editingJudge?.id && assignments.length === 1 ? editingJudge.id : `${assign.activityId}_${baseJudge.judgeName}_${assign.stageScope}`, 
                  activityId: assign.activityId,
                  role: assign.role,
                  stageScope: assign.stageScope,
                  clusterKey: clusterKey || '',
                  clusterLabel: clusterLabel || '',
                  // Pass originalId if editing existing record to handle name changes
                  originalId: editingJudge?.id
              };
                  ...
                  if (viewScope === 'area') {
                      ...
                      if (clusterFilter) {
                          ...
                          Object.values(group.clusters).forEach(c => {
                              // Filter by user role if needed for printing
                              if ((isGroupAdmin || isUserOrSchoolAdmin) && c.judges.length > 0 && c.judges[0].clusterKey !== userClusterID) return;
                                            ...
                                            <div className="flex items-center justify-between mb-1">
                                                ...
                                                <div className="flex items-center">
                                                    <input 
                                                        ...
                                                        id="isExternal"
                                                        checked={isExternal}
                                                        onChange={(e) => setIsExternal(e.target.value === 'true' ? true : e.target.checked)}
